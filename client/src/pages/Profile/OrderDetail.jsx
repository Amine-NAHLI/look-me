...
const OrderDetail = () => {
  ...
  const { data: order, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await axios.get(`http://localhost:5000/api/orders/my-orders/${id}`, {
          ...
          <div className="lg:col-span-12">
             <div className="bg-white rounded-[2.5rem] border border-gray-100 overflow-hidden shadow-sm">
                <div className="p-8 border-b border-gray-50 bg-gray-50/30 flex items-center justify-between">
                   ...
                   <span className="text-xs font-body text-gray-400 uppercase tracking-widest">{order.orderItems.length} articles</span>
                </div>
                   ...
                   {order.orderItems.map((item, idx) => (
                      <div key={idx} className="p-6 md:p-8 flex items-center gap-6 group hover:bg-gray-50/40 transition-colors">
                         ...
                         <div className="flex-grow">
                            ...
                            <p className="text-sm font-body text-gray-400">Quantité: {item.qty} × {formatPrice(item.price)}</p>
                         </div>
             ...
             <div className="bg-white rounded-[2.5rem] border border-gray-100 p-8 shadow-sm h-full">
                ...
                <div className="space-y-4">
                   <div className="flex flex-col bg-gray-50/50 p-6 rounded-3xl">
                      <p className="font-heading font-bold text-gray-900 mb-1">{order.customerName || order.user?.firstName}</p>
             ...
             <div className="bg-[#0A0A0A] text-white rounded-[2.5rem] p-8 shadow-2xl h-full">
                ...
                <div className="space-y-5">
                   <div className="flex justify-between items-center text-sm font-body text-gray-400">
                      ...
                      <span className="text-white font-bold">{formatPrice(order.subtotal || order.totalPrice - order.deliveryFee)}</span>
                   </div>
                   <div className="flex justify-between items-center text-sm font-body text-gray-400">
                      <span>Livraison</span>
