...
export default function LegalPage() {
  ...
  return <main className="mx-auto max-w-3xl px-5 py-16 md:py-24"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--primary)]">LOOKME</p><h1 className="mt-3 font-heading text-4xl">{title}</h1><p className="mt-8 leading-8 text-[var(--gray)]">{text}</p><p className="mt-8 rounded-[var(--radius)] border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950">Document modèle, à faire relire par un professionnel du droit avant publication.</p><Link className="mt-10 inline-block text-sm font-semibold text-[var(--primary)] underline" to="/">Retour à la boutique</Link></main>
}
