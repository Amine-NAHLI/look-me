...
import { slideInRight, scaleIn } from '../utils/animations';
...
const ProductDetails = () => {
  ...
  const { data: similarProducts } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/products', { params: { category: product?.category?._id, limit: 5 } });
      return data.items.filter(p => p._id !== id).slice(0, 4);
    },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-[120px] mb-[120px]">
        ...
        <div className="flex flex-col gap-4 relative">
          ...
          >
            {product.image ? (
               <img 
                  src={getImageUrl(product.images?.[activeImage] || product.images?.[0] || product.image)}
                 alt={product.name} 
          ...
          <div className="grid grid-cols-4 gap-4">
            {[...Array(4)].map((_, i) => (
              ...
              >
                {product.images?.[i] || product.images?.[0] || product.image ? <img src={getImageUrl(product.images?.[i] || product.images?.[0] || product.image)} alt="" className="w-full h-full object-cover opacity-80 hover:opacity-100 transition-opacity" /> : <div/>}
              </motion.button>
            ))}
          </div>
        </div>
