...
function ProductEditor({ id, initialProduct }) {
  ...
  const addImage = (url) => { if (!url) return; const normalized = typeof url === 'string' ? url.trim() : ''; if (!normalized) return; if (form.images.length >= 8) return toast.error('Maximum de 8 images'); setForm((current) => ({ ...current, images: [...current.images, normalized] })); }
