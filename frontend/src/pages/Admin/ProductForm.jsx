...
const emptyProduct = { name: '', description: '', category: '', price: '', compareAtPrice: '', sku: '', stock: 0, status: 'active', featured: false, images: [] }
...
const toForm = (product) => product ? { ...emptyProduct, ...product, price: String(product.price), compareAtPrice: product.compareAtPrice ? String(product.compareAtPrice) : '', stock: product.stock ?? 0, images: product.images || [], category: product.categoryId } : emptyProduct
...
function ProductEditor({ id, initialProduct }) {
  ...
  const submit = (event) => { event.preventDefault(); save.mutate({ ...form, price: Number(form.price), compareAtPrice: form.compareAtPrice === '' ? undefined : Number(form.compareAtPrice), stock: Number(form.stock) }) }
  return <main className="mx-auto max-w-5xl p-5 md:p-8"><header className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-[var(--primary)]">Catalogue</p><h1 className="mt-2 font-heading text-4xl">{editing ? 'Modifier le produit' : 'Nouveau produit'}</h1></div><Link className="text-sm font-semibold text-[var(--primary)] underline" to="/admin/products">Retour</Link></header>
    <form className="mt-8 space-y-8" onSubmit={submit}><section className="grid gap-5 rounded-[var(--radius-md)] border border-[var(--border)] bg-white p-5 md:grid-cols-2"><h2 className="col-span-full font-heading text-2xl">Informations</h2><label className="text-sm font-medium">Nom<input required name="name" value={form.name} onChange={change} className={field} /></label><label className="text-sm font-medium">Catégorie<select required name="category" value={form.category} onChange={change} className={field}><option value="">Sélectionner…</option>{categories.data?.filter((category) => category.status === 'active').map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><label className="col-span-full text-sm font-medium">Description<textarea required name="description" minLength="10" value={form.description} onChange={change} rows="6" className={field} /></label></section>
      <section className="grid gap-5 rounded-[var(--radius-md)] border border-[var(--border)] bg-white p-5 md:grid-cols-2"><h2 className="col-span-full font-heading text-2xl">Prix et inventaire</h2><label className="text-sm font-medium">Prix (MAD)<input required name="price" type="number" min="0" step="0.01" value={form.price} onChange={change} className={field} /></label><label className="text-sm font-medium">Ancien prix (facultatif)<input name="compareAtPrice" type="number" min="0" step="0.01" value={form.compareAtPrice} onChange={change} className={field} /></label><label className="text-sm font-medium">SKU<input name="sku" value={form.sku || ''} onChange={change} className={field} /></label><label className="text-sm font-medium">Stock<input required name="stock" type="number" min="0" step="1" value={form.stock} onChange={change} className={field} /></label><label className="text-sm font-medium">Visibilité<select name="status" value={form.status} onChange={change} className={field}><option value="active">Actif</option><option value="archived">Archivé</option></select></label><label className="flex items-center gap-3 self-end pb-3 text-sm font-medium"><input name="featured" type="checkbox" checked={form.featured} onChange={change} /> Mettre en avant</label></section>
      <section className="rounded-[var(--radius-md)] border border-[var(--border)] bg-white p-5"><h2 className="font-heading text-2xl">Images</h2><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{form.images.map((image, index) => <div key={`${image}-${index}`} className="relative"><img src={image} alt="" className="aspect-[4/5] w-full rounded object-cover" /><button type="button" onClick={() => setForm((current) => ({ ...current, images: current.images.filter((_, imageIndex) => imageIndex !== index) }))} className="mt-2 text-sm text-red-700 underline">Retirer</button></div>)}<ImageUploader onUploadComplete={(url) => addImage(url)} /></div><div className="mt-5 flex flex-col gap-2 sm:flex-row"><label className="sr-only" htmlFor="image-url">URL d'image</label><input id="image-url" value={imageUrl} onChange={(event) => setImageUrl(event.target.value)} className={field} placeholder="https://…" /><button type="button" onClick={() => addImage()} className="rounded border border-[var(--border)] px-4 py-2 text-sm font-semibold">Ajouter une URL</button></div></section>
