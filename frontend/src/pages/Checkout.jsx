  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      const orderData = {
        ...
        shippingAddress: { fullName: shippingAddress.fullName, phone: shippingAddress.phone, addressLine1: shippingAddress.address, city: shippingAddress.city, postalCode: shippingAddress.postalCode || undefined },
        idempotencyKey: crypto.randomUUID()
      };

      const { data } = await api.post('/orders', orderData);

      if (data.guestAccessToken) setGuestOrderToken(data.order._id, data.guestAccessToken);
      clearCart();
