        ...
        return <motion.article layout variants={scaleIn} key={product.id} whileHover={{ y: -4 }} transition={{ duration: 0.2 }} className="group relative">
          <Link to={`/product/${product.id}`} className="block" aria-label={`Voir ${product.name}`}>
            <div className="relative mb-3 aspect-[4/5] overflow-hidden bg-[#F5F5F5]">
              ...
              {product.featured && <span className="absolute left-3 top-3 rounded-full border border-white/40 bg-white/40 px-3 py-1 text-[9px] font-bold uppercase tracking-widest text-[#1C1C1C] backdrop-blur-md">Sélection</span>}
              {product.stock <= 0 && <span className="absolute left-3 top-3 rounded-full bg-black/80 px-3 py-1 text-[9px] font-bold uppercase tracking-widest text-white backdrop-blur-md">Épuisé</span>}
            </div>
